...
no_of_readings=300  # totally how many readings will be done
sleep_time_seconds=1 # the interval of neighbouring readings.
...
for n in range(0, no_of_readings-1):
    ...
    temp=ser.readline()
    reading_scale[n]=temp
    #  reading_time[n]=temp[3]
    fid.write(time.strftime("%d/%b/%Y %H:%M:%S")+reading_scale[n])
    time.sleep(sleep_time_seconds)
    if (n % 10) ==0 :
        print(time.strftime("%d/%b/%Y %I:%M:%S")+reading_time[n]+reading_scale[n])
    time.sleep(sleep_time_seconds)
